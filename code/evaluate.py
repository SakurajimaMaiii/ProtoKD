...
def test_single_case(net, image, stride, patch_size, num_classes=1):
    ...
    could be used for validation and test
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path',type=str,default='../log/0409baseline/model/epoch_880.pth',
                          help='test model path')
    parser.add_argument('--num_cls',type=int,default=4,help='the number of class')    
    parser.add_argument('--test_modality', nargs='+', type=int, default=1, help='test modality contain images ROIs to test one of [0,1,2,3]')
    parser.add_argument('--output_path',type=str,default='../results/PMKL_T1',help='results save path') 
    parser.add_argument('--save_vis',action='store_true',help='save vis results or not')
    ...
    logging.basicConfig(filename=output_path+'/results.txt', level=logging.INFO,format='%(message)s', datefmt='%S')
    ...
    with torch.no_grad():
        for idx,data_path in enumerate(tqdm(imglist,ncols=70,colour='#9999FF')):
            ...
            if args.save_vis:
                ...
                convert_to_sitk(image,output_path+'/%s'%name_vol,modality=test_modality)
                convert_to_sitk(label.astype(np.uint8),output_path+'/%s'%name_vol+'/label.nii.gz')
                convert_to_sitk(predict.astype(np.uint8),output_path+'/%s'%name_vol+'/predict.nii.gz')
    ...
    logging.info('Average dice is {}.'.format((dice_mean[0]+dice_mean[1]+dice_mean[2])/3.0))
    logging.info('Average hd is {}.'.format((hd_mean[0]+hd_mean[1]+hd_mean[2])/3.0))
    logging.info('Average spe is {}.'.format((spe_mean[0]+spe_mean[1]+spe_mean[2])/3.0))
    logging.info('Average sen is {}.'.format((sen_mean[0]+sen_mean[1]+sen_mean[2])/3.0))
